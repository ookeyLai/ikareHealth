...
<link rel="stylesheet" type="text/css"
	href="jquery/jquery.timepicker.css" />
...
<body>
    ...
    <?=$_WELCOME;?>&nbsp; <?=$name;?>&nbsp;&nbsp; [<a
					href="login.php?action=logout"><?=$_LOGOUT;?></a>]
				</td>
			</tr>
		</table>
	</div>
		...
		<tr>
			<td width="200" valign="top">
				<ul id="menu1">
					...
					<hr>
					<li
						class="<?=($page=='logList.php'&&!$itemID?'ui-state-error':''); ?>"><a
						href="index.php"><?=$_ALL;?></a></li>
  <?php
  ...
  <?php
		...
		?>
				...
				<ul id="menu3">
					...
					<hr>
					<li class="<?=($page=='userAccount.php'?'ui-state-error':''); ?>"><a
						href="?page=userAccount"><?=$_ACCOUNT;?></a></li>
					<li class="<?=($page=='userDevice.php'?'ui-state-error':''); ?>"><a
						href="?page=userDevice"><?=$_DEVICE;?></a></li>
					<li class="<?=($page=='userCategory.php'?'ui-state-error':''); ?>"><a
						href="?page=userCategory"><?=$_CATEGORY;?></a></li>
					<li class="<?=($page=='userArticle.php'?'ui-state-error':''); ?>"><a
						href="?page=userArticle"><?=$_ARTICLE;?></a></li>
				</ul> <br />
	<?php
	if (count ( $agroupID ) > 1) {
...
<br />
				<div id='sysdiv' style="display: none">
					<ul id="menu4">
						...
						<hr>
						<li class="<?=($page=='accountList.php'?'ui-state-error':''); ?>"><a
							href="?page=accountList"><?=$_ACCOUNT;?></a></li>
						<li class="<?=($page=='itemList.php'?'ui-state-error':''); ?>"><a
							href="?page=itemList"><?=$_ITEM;?></a></li>
						<li
							class="<?=($page=='itemArticleList.php'?'ui-state-error':''); ?>"><a
							href="?page=itemArticleList"><?=$_ARTICLE;?></a></li>
						<li class="<?=($page=='categoryList.php'?'ui-state-error':''); ?>"><a
							href="?page=categoryList"><?=$_CATEGORY;?></a></li>
						<li class="<?=($page=='deviceList.php'?'ui-state-error':''); ?>"><a
							href="?page=deviceList"><?=$_DEVICE;?></a></li>
					</ul>
				</div>
			</td>
			<td width="87%" height="480" align="center" valign="top"
				bgcolor="#CCCCCC">
				<div id="main_content">
        <?php
								include($page);
